...
const Event = ({ eventInfo }) => {
    ...
    return (
        <div className="flex flex-wrap bg-gray-800 text-white rounded-lg max-w-lg min-h-[100px] text-left p-3">
            <div className="w-full md:w-2/3 p-2 rounded-md" style={
                ...
                {/* Title of the event */}
                <h2 className="text-base text-slate-2S0 mb-2">{startDate} {startDate !== endDate && endDate !== "Invalid Date" ? (` - ${endDate}`) : ""}</h2>
                ...
                <p className="text-base break-words">{eventInfo.extendedProps.description}</p>
            </div>
            ...
            <div className="w-full md:w-1/3 text-center flex flex-col text-md">
                <div className="flex flex-col gap-8 flex-1 justify-center m-2">
                    <div className="flex flex-col gap-1">
                        <p className=" m-0"><span className="font-bold">Start: </span>{eventStartTime}</p>

                        {eventEndTime !== "Invalid Date" && <p className=" m-0"><span className="font-bold">End: </span>{eventEndTime}</p>}
                        <p className=" m-0"><span className="font-bold">Location: </span>{eventInfo.extendedProps.location || "TBD"}</p>
                    </div>
                    {/* {eventInfo.url ? (
